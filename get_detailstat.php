...
<body>
	<a href="./get_allstat.php">กลับไปก่อนหน้า</a>
	<?php
		...
		if(isset($_GET['url']))
		{
			...
			$stat = str_replace(array('class="fblike"','class=" fb_reset"','class="league"'),array('class="unshow"','class="unshow"','class="unshow"'),$stat);
...
<script>
	$(window).load(function(){
		$("img").each(function(){
			...
			if(old_src.indexOf("http://") < 0) {
				if(old_src.indexOf("/") != 0) old_src = "/" + old_src;
				$(this).attr("src","http://www.olegoal.com"+old_src);
			}
		});
		$('.unshow').remove();
		$('.clear').remove();
		$('div[style="width:640; display:block;"]').remove();
		$('img[style="margin-right: 10px;"]').remove();
	});
</script>
</body>